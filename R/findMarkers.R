...
.logBH <- function(log.p.val) {
    log.p.val_definite <- na.omit(log.p.val)
    o <- order(log.p.val_definite)
    repval <- log.p.val_definite[o] + log(length(o)/seq_along(o))
    repval <- rev(cummin(rev(repval)))
    output <- rep(NA, length(log.p.val))
    output[setdiff(seq_along(output),attr(log.p.val_definite, "na.action"))][o] <- repval
    output
}
...
.choose_logFC <- function(all.p, all.logFC, pval.type, min.prop){
  if (pval.type == "any"){
    col_index <- apply(all.p, 1, function(x) which.min(c(x,Inf)))
  } else if (pval.type == "all") {
    col_index <- apply(all.p, 1, function(x) which.max(c(x,-Inf)))
  } else {
