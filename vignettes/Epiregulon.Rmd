...
```{r}
addTFMotifInfo(archr_path = archR_project_path, grl)
```
...
```{r}
regulon <- getRegulon(archr_path = archR_project_path, p2g, keep_id=FALSE)
head(regulon)
```
...
```{r}
markers <- getSigGenes(da_list, fdr_cutoff = 1.5, logFC_cutoff = -100, order = "summary.logFC", decreasing = T)
head(markers)
```
