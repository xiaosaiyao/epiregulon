...
```{r}
regulon <- getRegulon(archr_path = archR_project_path, p2g)
head(regulon)
```
...
```
Trim regulon to use 4 TFs only for demonstration purposes
...
## 6 Differential TF activity test
...
```{r}
markers <- getSigGenes(da_list, fdr_cutoff = 0.05, logFC_cutoff = NULL, order="FDR")
head(markers)
```

## 7. Visualizing TF activities
