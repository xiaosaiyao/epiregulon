addPadding <- function(gr) {
  # extend regions covered by chip-seq to generate partially overlapping peaks
  if (length(gr) < 2) {
    return(resize(gr, width=width(gr) + 1000, fix="center"))
  }
  gr <- reduce(gr)
  ...
  distances_downstream <- sapply(floor(distances_downstream / 2), min, 500) 
  
  # avoid merging adjacent binding regions during subsequent operations
...
create_peak_sequence <- function(gr) {
  # narrow range to the maximum size of 501 bp
...
select_hits <- function(o) {
  # choose random region if multiple regions overlap with the same TF binding site
  if (length(o) == 0) {
    return(numeric(0))
  }
...
grl_binding <- GRangesList(lapply(grl_binding, function(x) 
  {x[sample(length(x), round(0.2 * length(x)))]}))
...
unoccupied_sequences <- unlist(GRangesList(
  S4Vectors::Reduce(GenomicRanges::intersect, grl_binding_negative)
))
...
for (i in 1:nrow(pairs)) {
  ...
  shared_regions[[i]] <- GenomicRanges::intersect(grl_binding[[pairs[i, 1]]],
                                                  GenomicRanges::intersect(grl_binding[[pairs[i, 2]]], unoccupied_by_other_tfs))
}
  ...
  if (n_peaks > 0) {
    ...
    for (region in regions) {
      ...
      overlaps <- rbind(overlaps,
                        data.frame(
                          idxATAC=length(peaks),
                          idxTF=pairs[i, ],
                          tf=names(grl_binding)[pairs[i, ]]
                        ))
    }
  }
}
...
for (i in selected_tfs) {
  if (is.null(unoccupied_by_other_tfs[[i]]))
    unoccupied_by_other_tfs[[i]] <- S4Vectors::Reduce(GenomicRanges::intersect, grl_binding_negative[-i])
}

# clean by subtracting sequences covered by other TFs
grl_binding_unique <- GRangesList(
  lapply(selected_tfs, function(i, grl, grl_uo)
    GenomicRanges::intersect(grl[[i]], grl_uo[[i]]), grl=grl_binding, grl_uo=unoccupied_by_other_tfs)
)
grl_binding_extended <- GRangesList(lapply(grl_binding_unique, addPadding))
grl_binding_extended <- GRangesList(lapply(grl_binding_extended, function(x)
  GenomicRanges::intersect(x, unoccupied_sequences))) # extension should not overlap with bindings sites of other TFs

# glue paddings with cleaned TF binding sites
grl_binding_extended <- GRangesList(
  lapply(selected_tfs, function(i, grl, grl_ex)
    reduce(c(grl[[which(selected_tfs == i)]], 
             grl_ex[[which(selected_tfs == i)]])), grl=grl_binding_unique, grl_ex=grl_binding_extended)
)
grl_binding_extended <- GRangesList(lapply(selected_tfs, function(i, grl, grl_ex) {
  o <- findOverlaps(grl_ex[[which(selected_tfs == i)]], grl[[i]])
  grl_ex[[which(selected_tfs == i)]][select_hits(o)]}, 
...
for (tf_id in seq_along(grl_binding_extended)) {
  ...
  if (new_peaks_n > 0) {
    ...
    overlaps <- rbind(overlaps,
                      data.frame(
                        idxATAC=length(peaks) + length(new_peaks) + seq_len(new_peaks_n),
                        idxTF=original_TF_ID,
                        tf=names(grl_binding)[original_TF_ID]
                      )
    )
...
if (length(to_narrow) > 0) {
  new_start <- sapply(width(new_peaks[to_narrow]) - 501, function(x)
    sample(0:x, 1)) + start(new_peaks[to_narrow])
...
peakMatrix <- SingleCellExperiment(assays=list(counts=matrix(sample(c(0, 1), length(peaks) * 200, replace=TRUE), length(peaks), 200)), 
                                   rowRanges=peaks)
...
peak_gene_links <- data.frame(
  idxATAC=sample(all_peaks, 4000, replace=TRUE),
  chr="chr1",
