...
geneExpMatrix <- matrix(rnorm(length(expression_means), mean=expression_means),
                        ncol=n_cells,
...
for (peak_idx in unique(regulatory_pairs[,2])){
    ...
    target_expression <- Matrix::colSums(expression_means[target_genes,,drop=FALSE])
...
peakMatrix_sce <- SingleCellExperiment(assay=list(counts=peakMatrix),
                                       rowRanges=peak.ranges)
geneExpMatrix_sce <- SingleCellExperiment(assay=list(counts=geneExpMatrix),
                                          rowRanges=gene.ranges)
cellNum <- optimizeMetacellNumber(peakMatrix_sce,
                                  geneExpMatrix_sce,
                                  reducedDim=t(as.matrix(geneExpMatrix)),
                                  exp_assay="counts",
                                  peak_assay="counts",
                                  subsample_prop=0.1,
                                  n_iter=2,
